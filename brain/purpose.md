...
- **Lightweight Text Badges**: Mode indicators (`NORMAL`, `INSERT`, `:CMD`) and docked utilities (`AI Agent`) use clean, colored typography without clunky background boxes or borders.
- **Clean Chat Dynamics**: In the AI panel, user messages anchor cleanly to the right, assistant messages to the left, paired with minimal circular avatars and readable bubbles.
